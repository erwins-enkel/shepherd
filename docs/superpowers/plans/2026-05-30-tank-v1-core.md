...
**ToS invariant (do not violate in any task):** Shepherd never runs `claude -p` or the Agent SDK. It only `start`s an interactive `claude` under herdr and `attach`es a real PTY. Task naming uses **ollama**, never Anthropic.
...
export const config = {
  ...
  dbPath: process.env.SHEPHERD_DB ?? `${process.env.HOME}/.tank/tank.db`,
...
- **Out of scope here (Plan 2):** all `src/` UI, the Shepherd HUD (`theme.css`, xterm theme, components), read-only second-viewer UX. Tracked for Plan 2.
...
Plan 2 — **Shepherd HUD UI** (SvelteKit 5 + Tailwind 4 + xterm.js): port `mockup/hud.html` into `theme.css` tokens + components (TopBar, Herd/UnitRow/StatusPip, Viewport, NewTask), wire to `/api/sessions`, `/events`, `/pty/:id`, including the read-only second-viewer + "Take over" UX. Backend from Plan 1 is its live API.
