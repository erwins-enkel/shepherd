...
struct SettingsFakeResponse: Sendable {
  var statusCode: Int
...
struct SettingsRecordedRequest: Sendable {
  let method: String
  let path: String
  let query: String?
...
enum SettingsFakeServerError: Error, Equatable {
  case noRoute(String)
}
...
final class SettingsFakeServer: Sendable {
  ...
  init() {
    ...
    SettingsFakeServerRegistry.shared.register(registryKey)
  }
  ...
  func on(
    _ method: String,
    _ path: String,
    _ handler: @escaping @Sendable (SettingsRecordedRequest) throws -> SettingsFakeResponse
  ) {
    SettingsFakeServerRegistry.shared.setHandler(
  ...
  func stub(_ method: String, _ path: String, status: Int, json: Data) {
    on(method, path) { _ in SettingsFakeResponse(statusCode: status, body: json) }
  }

  /// Every request the fake has served, oldest first.
  func requests() -> [SettingsRecordedRequest] {
    SettingsFakeServerRegistry.shared.requests(registryKey)
  }
  ...
  func urlSession(
    ...
    configuration.protocolClasses = [SettingsFakeURLProtocol.self]
    configuration.httpAdditionalHeaders = [Self.sessionHeader: UUID().uuidString]
    return URLSession(configuration: configuration)
  }

  func tearDown() {
    SettingsFakeServerRegistry.shared.unregister(registryKey)
  }
}
...
/// cannot be an actor.
private final class SettingsFakeServerRegistry: Sendable {
  static let shared = SettingsFakeServerRegistry()

  private struct Entry {
    var handlers: [String: @Sendable (SettingsRecordedRequest) throws -> SettingsFakeResponse] = [:]
    var recorded: [SettingsRecordedRequest] = []
  ...
  func setHandler(
    ...
    handler: @escaping @Sendable (SettingsRecordedRequest) throws -> SettingsFakeResponse
  ) {
    entries.withLock { $0[key]?.handlers[route] = handler }
  }

  func requests(_ key: String) -> [SettingsRecordedRequest] {
  ...
  /// the lock is held.
  func take(_ request: SettingsRecordedRequest, host: String)
    -> (@Sendable (SettingsRecordedRequest) throws -> SettingsFakeResponse)?
  {
    entries.withLock { entries in
...
/// The `URLProtocol` that serves `SettingsFakeServer`.
final class SettingsFakeURLProtocol: URLProtocol {
  private let deliveryQueue = DispatchQueue(label: "SettingsFakeServer.delivery")
  ...
  override class func canInit(with request: URLRequest) -> Bool {
    ...
    return SettingsFakeServerRegistry.shared.knows(host: host)
  }
  ...
  override func startLoading() {
    guard let url = request.url, let host = url.host() else {
      fail(SettingsFakeServerError.noRoute("<no host>"))
      return
    }
    ...
    let session = request.value(forHTTPHeaderField: SettingsFakeServer.sessionHeader)
    var headers = request.allHTTPHeaderFields ?? [:]
    if headers["Cookie"] == nil, let session,
      let cookies = SettingsFakeServerRegistry.shared.cookieHeader(host: host, session: session)
    {
      headers["Cookie"] = cookies
    }
    let recorded = SettingsRecordedRequest(
      method: request.httpMethod?.uppercased() ?? "GET",
      path: url.path(percentEncoded: false),
      query: components?.query,
      headers: headers,
      body: request.httpBody ?? request.httpBodyStream.map { settingsDrainRequestBody($0) }
    )

    guard let handler = SettingsFakeServerRegistry.shared.take(recorded, host: host) else {
      fail(SettingsFakeServerError.noRoute("\(recorded.method) \(recorded.path)"))
      return
    }
    ...
    do {
      ...
      if let setCookie = fake.headers["Set-Cookie"], let session {
        SettingsFakeServerRegistry.shared.absorb(setCookie: setCookie, host: host, session: session)
      }
...
func settingsDrainRequestBody(_ stream: InputStream, timeout: TimeInterval = 2) -> Data {
