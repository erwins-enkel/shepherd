...
The defining constraint is the ToS-compliance model, not a footnote: Shepherd only **observes** (reads the terminal and agent status) and **steers** (injects keystrokes into the live pane). By default it uses no Agent SDK or `claude -p`; if a feature cannot be done by typing into a real terminal, it does not ship. This model is Shepherd's **stated position / design stance**, not Anthropic-confirmed settled fact — whether interactive keystroke-puppeting for automated work is permitted is the open question in audit **R1** (see the [ToS compliance audit](docs/research/claude-anthropic-tos-compliance-audit.md) and the [auth-path evaluation](docs/research/tos-position-and-auth-paths.md), which also records the opt-in API-key / metered footings). This constraint shapes the product's soul, and the UI should make that interactive-terminal reality felt, not hidden behind abstraction.
