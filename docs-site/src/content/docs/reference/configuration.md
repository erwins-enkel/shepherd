...
| `SHEPHERD_TRUST_ISSUE_AUTHORS` | `0` (off) | Opt-in escape hatch for the fail-closed author-trust gate on **autonomous** (`auto=true`) drain. On forges that structurally can't supply a GitHub-style author association (non-GitHub — Gitea/local), autonomous drain is otherwise silently disabled; set `1` to treat issue authors there as trusted. Scoped to non-GitHub only — on GitHub, where author trust is verifiable, a miss or untrusted author still refuses. See [Security](/reference/security/) |
