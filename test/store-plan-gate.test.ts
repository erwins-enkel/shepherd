...
test("putPlanGate/getPlanGate round-trips blocks with inferred flag intact", () => {
  ...
  const blocks: VisualBlock[] = [
    { type: "question-form", questions: [{ id: "q1", label: "Goal?", kind: "text" }] },
    { type: "data-model", entities: [], inferred: true },
  ];
  ...
  const dm = got?.blocks?.find((b) => b.type === "data-model") as any;
...
test("snapshotPlanGates includes blocks for stored gate", () => {
  const s = new SessionStore(":memory:");
  const blocks: VisualBlock[] = [
    { type: "question-form", questions: [{ id: "q2", label: "Scope?", kind: "text" }] },
  ];
