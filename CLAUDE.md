...
1. Add one fragment file in `ui/src/lib/feature-announcements/entries/` named `v<version>-<id>.ts` (for example `v1.41.0-quick-filter.ts`) with a default export that satisfies `FeatureAnnouncement`: `id` (stable kebab slug), `sinceVersion` (the release it ships in), `titleKey` + `bodyKey`. **`<version>` = the NEXT (unreleased) version — run `bun run next-version` to get it; NEVER read `package.json`.** Between releases `package.json` holds the _last released_ version (release-please only bumps it when its release PR merges), so stamping that makes the entry invisible forever: `feature-gate.ts` only surfaces `sinceVersion > the user's lastSeen`, and an upgraded user already saw the last release. Use the same string for both the filename and the `sinceVersion` field. **No sequence-number prefix** — the filename is just `v<version>-<id>.ts`. The `id` is globally unique (enforced by the dup-id guard), so two concurrent PRs never collide on a filename; a shared `NNNN-` counter would reintroduce the very merge-conflict hotspot this split removed. Entries sort by `sinceVersion` then filename, so a curated cross-file order is neither needed nor available.
...
**Version gate:** `scripts/check-announcement-versions.mjs` (also in **PR hygiene** CI + pre-push) closes the version footgun above: for every entry fragment _added_ in the branch's range it fails if `sinceVersion` is `<=` the last released version (from `.release-please-manifest.json`) — i.e. an entry that would never surface — or if the filename's `v<version>-` prefix disagrees with the `sinceVersion` field. Fix by running `bun run next-version` and using that value for both. Only added files are checked, so editing a historical entry never trips it.
