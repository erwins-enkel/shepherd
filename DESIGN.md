...
- **Shape:** Gently squared (2px radius) — the default for a **standalone** button. Square enough to read as instrument hardware, never pill-shaped. Two softenings are sanctioned, neither reaching the forbidden full pill (999px): a _status chip_ steps to the 6px chip radius (see Status Chip below), and an interactive control that sits **inside a status/action chip row** may adopt that same 6px radius so the row reads as one cohesive chip set. A button standing on its own outside such a row stays 2px.
...
- **When:** A compact **functional-status** readout in a chip row — git / CI / PR / critic / ready state. It is a distinct control class from a structural **Panel** (square, 0 radius) and the inline **Badge** (the smallest 2px micro-label embedded in running text or a table cell). It differs from a **Button** by _role_ — a chip reports state, a button acts — but the two are no longer forced apart by _radius_: inside a status/action chip row an interactive control may take the chip's 6px radius (see Chip-row cohesion below). Reach for a chip when a row of machine-state values must read at a glance and stay thumb-reachable.
...
- **Chip-row cohesion (interactive controls):** When a status/action chip row also carries interactive controls — MERGE, READY, the critic verdict chip, ⚙ AUTO, review — those controls may adopt the same 6px chip radius so the whole row reads as **one cohesive chip set**, rather than a 6px-chip + 2px-button mix. This is a deliberate relaxation of the shape-encodes-interactivity signal (a _standalone_ button is still 2px; see Buttons) — inside a chip row, row cohesion wins. Interactivity is then carried by the cues that already separate action from readout: the action label, the hover-lift (a resting status chip never lifts), and the amber primary treatment. The full pill (999px) stays forbidden here as everywhere.
- **Top-bar right cluster (cross-surface cohesion):** The header's four bordered right-cluster badges — held / update / what's-new (incl. the compact what's-new pip) — and the ⌘K search trigger share the same 6px chip radius rung so the header's boxed controls and the GitRail row read as one corner language, not two design systems. This is a **radius-only** alignment: each control keeps its own existing chrome (its fill — transparent, tinted, or inset — the fixed `--topbar-ctl-h` height, its sans-serif label font (vs the chip recipe's mono), and the update badge's amber attention pulse) and is **not** converted to the compact `panel-2` status-chip recipe. The borderless gear glyph and the square (0-radius) status tallies are outside this set — they already share the `--topbar-ctl-h` **height** token but not the radius rung, and are unchanged. Only the corner rung is newly shared.
- **Two scoped allowances beyond a literal chip row** (used by the Viewport identity row / task bar, where a control relates to the git chip strip it sits with but is not physically _in_ a chip row). Each is justified on its own footing, **not** by pretending the identity row is a contiguous chip cluster:
  - **Standalone ghost chip** — a read-only _identity label_ (the `TASK-XX` designation) may take the 6px chip radius as a **hue-less, dot-less** boxed label. It is distinct from both the inline 2px **Badge** (kept for status tokens) and the semantic **Status Chip** (which carries hue + dot): the ghost chip is shape-only, reserved for the primary identity label, and never borrows a status color.
  - **Chip-radius disclosure control** — the interactive control that _discloses_ a chip strip (the git-rail foldout) may adopt the 6px radius to rhyme with what it opens, and may retain a subordinate **inset glow** (`inset 0 0 18px -10px`) on its semantic states as the disclosure's emphasis. This is a scoped deviation from the no-halo rule for status chips; it stays subordinate (a tight −10px inner tint, never a broadcast halo) so the blocked-agent pip's pulse + oxblood wash remain the loudest red (Four-Light Rule).
  - **What stays a Badge:** read-only status pips that live among other 2px badges — the ready pip and the plan-gate / autopilot status badges in the identity row — remain the **2px Badge** primitive, so a lone 6px chip is never stranded between 2px siblings.
...
- **Do** keep structural panels square (0 radius), use the 2px radius for standalone buttons and controls and the 6px chip radius for status chips — and, for cohesion, a control **inside a status/action chip row** may take that same 6px chip radius — and reserve 12px for rising bottom sheets. The full pill (999px) stays forbidden.
