  ...
  "../../../node_modules/.pnpm/@sinclair+typebox@0.31.28/node_modules/@sinclair/typebox/typebox.js"(
    ...
    (function (TypeGuard2) {
      ...
      function TBoolean(schema) {
        return (
          TKindOf(schema, "Boolean") && schema.type === "boolean" && IsOptionalString(schema.$id)
        );
      }
      ...
      function TIntersect(schema) {
        return (
          ...
          schema.allOf.every((schema2) => TSchema(schema2) && !TTransform(schema2)) &&
        ...
        return (
          ...
          ((schema2) => {
            ...
            return (
              ...
              TSchema(schema2.patternProperties[keys[0]])
            );
          })(schema)
        );
      }
      ...
      function TRef(schema) {
        return (
          TKindOf(schema, "Ref") && IsOptionalString(schema.$id) && ValueGuard.IsString(schema.$ref)
        );
      }
      ...
      function TSymbol(schema) {
        return (
          TKindOf(schema, "Symbol") && schema.type === "symbol" && IsOptionalString(schema.$id)
        );
      }
      ...
      function TTuple(schema) {
        return (
          ...
          ((ValueGuard.IsUndefined(schema.items) &&
            ...
            schema.minItems === 0) ||
            (ValueGuard.IsArray(schema.items) && schema.items.every((schema2) => TSchema(schema2))))
        );
      }
      ...
      function TUnionLiteral(schema) {
        return (
          TUnion(schema) &&
          schema.anyOf.every((schema2) => TLiteralString(schema2) || TLiteralNumber(schema2))
        );
      }
                                                            ...
                                                            : TypeGuard.TUnion(left)
                                                              ...
                                                              : TypeGuard.TUnknown(left)
                                                                ...
                                                                : TypeGuard.TVoid(left)
                                                                  ? TVoid(left, right)
                                                                  : Throw(
                                                                      `Unknown left type operand '${left[exports.Kind]}'`,
                                                                    )
        );
      }
            ...
            : TypeGuard.TUnion(unionType)
              ? (() => {
                  const narrowed = unionType.anyOf.filter(
                    (inner) =>
                      TypeExtends.Extends(inner, excludedMembers) === TypeExtendsResult.False,
                  );
              ...
              return TemplateLiteralFinite.Check(expression)
                ? this.Object(
                    [...TemplateLiteralGenerator.Generate(expression)].reduce(
                      (acc, key2) => ({ ...acc, [key2]: TypeClone.Type(schema) }),
                      {},
                    ),
                    options,
                  )
                : this.Create({
                    ...
                    [exports.Kind]: "Record",
                    type: "object",
                    patternProperties: { [key.pattern]: TypeClone.Type(schema) },
                  });
            })()
            ...
            ? (() => {
                ...
                if (TypeGuard.TUnionLiteral(union)) {
                  const properties = union.anyOf.reduce(
                    (acc, literal) => ({ ...acc, [literal.const]: TypeClone.Type(schema) }),
                    {},
                  );
              ...
              : TypeGuard.TInteger(key) || TypeGuard.TNumber(key)
                ? (() => {
                    return this.Create({
                      ...
                      [exports.Kind]: "Record",
                      type: "object",
                      patternProperties: { [exports.PatternNumberExact]: TypeClone.Type(schema) },
                    });
                  })()
...
var sort = import_typebox.Type.Optional(
  import_typebox.Type.Union(
    [import_typebox.Type.Literal("asc"), import_typebox.Type.Literal("desc")],
    {
      title: "Sort keys",
...
var sortObjectKeys = (value, direction) => {
  ...
  const sorted = {};
  for (const [key, entry] of Object.entries(value).sort((a, b) => compare(direction)(a[0], b[0]))) {
...
var exportFiles = async ({ bundles, messages, variants, settings }) => {
  const files = {};
  for (const message of messages) {
  ...
  for (const locale in files) {
    ...
    const sortedMessages = sortDirection
...
function serializePattern(pattern) {
  ...
  for (const part of pattern) {
    switch (part.type) {
      ...
      case "markup-start":
        result += serializeMarkup("#", part.name, part.options, part.attributes, false);
        break;
      case "markup-end":
        result += serializeMarkup("/", part.name, part.options, part.attributes, false);
        break;
      case "markup-standalone":
        result += serializeMarkup("#", part.name, part.options, part.attributes, true);
        break;
      default:
...
function serializeMatcher(matches) {
  const parts = matches
    ...
    .map((match) =>
      match.type === "literal-match" ? `${match.key}=${match.value}` : `${match.key}=*`,
    );
  return parts.join(", ");
}
