...
import { miseClaudeState } from "./mise-claude";
...
// DIAGNOSE row and `buildWrappedArgv`'s `DISABLE_AUTOUPDATER=1` pin, through one TTL cache. The
// early kick is deliberate: the pin is read SYNCHRONOUSLY on the spawn path and reads `false`
// while cold, so warming it before the first plausible spawn is what makes it effective.
const checkMiseClaude = async () => void (await miseClaudeState(Date.now()));
setTimeout(timerTask("mise-claude", checkMiseClaude), 1_000);
setInterval(timerTask("mise-claude", checkMiseClaude), 6 * 60 * 60 * 1000);
...
const diagnostics = new DiagnosticsService({
  ...
  // auto-updater pin can never disagree. Wired here (not a ctor default) so no test ever
  // spawns `mise which claude` against the real host.
  readMiseClaude: () => miseClaudeState(Date.now()),
});
