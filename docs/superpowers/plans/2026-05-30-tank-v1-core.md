...
tank/
  ...
  src/
    ...
    pty-bridge.ts             # PtyBridge (node-pty ⇄ herdr attach)
...
/** Bridges one browser WS to a real PTY attached to a herdr terminal. */
export class PtyBridge {
  private pty: IPty | null = null;
  constructor(private terminalId: string, private ws: PtySocket) {}

  open(cols = 100, rows = 30): void {
    this.pty = spawn(config.herdrBin, ["agent", "attach", this.terminalId], {
      name: "xterm-color", cols, rows, env: process.env as Record<string, string>,
    });
    this.pty.onData((d) => this.ws.send(d));
    this.pty.onExit(() => this.ws.close());
  }
  write(data: string): void { this.pty?.write(data); }
  resize(cols: number, rows: number): void { this.pty?.resize(cols, rows); }
  close(): void { this.pty?.kill(); this.pty = null; }
}
```
    ...
    websocket: {
      ...
      message(ws, msg) {
        ...
        if (str.startsWith("\x00resize:")) {
          const [, c, r] = str.split(":");
          ws.data.bridge?.resize(Number(c), Number(r));
        } else ws.data.bridge?.write(str);
      },
      close(ws) {
...
```bash
git add src/pty-bridge.ts src/server.ts && git commit -m "feat: pty bridge + bun ws wiring (/pty, /events)"
```
