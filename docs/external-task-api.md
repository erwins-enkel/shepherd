...
| `images`        | `string[]`                                                                                                                                             | —        | ≤10 paths, each confined to the upload staging dir (see `POST /api/uploads`). Staged files may be images, PDF/markdown/text, or video (`mp4`/`mov`/`webm`/`m4v`); a video attachment adds an ffmpeg keyframe/audio-extraction note to the launch prompt, since an agent can't watch it directly |
...
- `POST /api/sessions/:id/scratchpad/upload[?path=<relDir>]` — upload an
  ...
  exceeds the upload size limit (250 MiB — the uniform `MAX_UPLOAD_BYTES` across
  every upload route, sized for screen recordings), and `404` on a
  missing/archived session or an out-of-root `path`. Both upload routes get a
  120 s idle budget so a slow phone upload isn't cut off mid-stream.
