...
struct DiffAnnotationLayout: Equatable, Sendable {
    struct LineKey: Hashable, Sendable {
        enum Side: Hashable, Sendable { case old, new }
        let path: String
        let side: Side
        let number: Int
    }
    ...
    var fileLevel: [String: [DiffNote]] = [:]
    /// `agent` notes anchored to one rendered line.
    var perLine: [LineKey: [DiffNote]] = [:]
    ...
    var hunks: [String: UnifiedPatch] = [:]

    static func partition(notes: [DiffNote], files: [DiffFile]) -> DiffAnnotationLayout {
