...
final class RepoBranchModel {
    enum Picker: Hashable { case repo, branch }
    var presentedPicker: Picker?
    ...
    private(set) var upstream: BranchStatus?
    private(set) var loadingBranches = false
    private(set) var upstreamLoading = false
    private(set) var repairingBase = false
    ...
    var baseOptions: [String] { branches.contains(baseBranch) ? branches : [baseBranch] + branches }
    ...
    func repairInitialCommit() async {
