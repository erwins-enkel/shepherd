...
1. Add one fragment file in `ui/src/lib/feature-announcements/entries/` named `v<version>-<id>.ts` (for example `v1.40.0-quick-filter.ts`) with a default export that satisfies `FeatureAnnouncement`: `id` (stable kebab slug), `sinceVersion` (the release it ships in), `titleKey` + `bodyKey`. **No sequence-number prefix** — the filename is just `v<version>-<id>.ts`. The `id` is globally unique (enforced by the dup-id guard), so two concurrent PRs never collide on a filename; a shared `NNNN-` counter would reintroduce the very merge-conflict hotspot this split removed. Entries sort by `sinceVersion` then filename, so a curated cross-file order is neither needed nor available.
