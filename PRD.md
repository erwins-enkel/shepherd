...
Derived from Creator Magic's "Tank" demo (yt: hXWwqPgexZU), re-architected for our stack
...
cut off **2026-06-15**. **Interactive terminal use was NOT banned.** Tank drives genuine
interactive `claude` sessions and only *observes/steers* them. That keeps subscription usage in
...
path on a sub. Tank's interactive-session substrate becomes the **compliant execution path** for
...
```
┌─ Tank Web/PWA (SvelteKit5 + Bun + Tailwind4) ─┐      browser + mobile
│   task list · status lights · xterm.js pane  │
│   PR/merge buttons · chat · usage · img DnD   │
└───────────────┬───────────────────────────────┘
                │ WS/SSE + REST
┌───────────────┴─ Tank orchestrator (Bun/TS) ─┐      the only NEW backend
...
| F13 | Self-building | Tank can build its own features and redeploy via CI |
...
Chat history + usage either in kanban-api SQLite or a small Tank-local SQLite (decide in spec).

## 9. Integration workstreams

1. **Tank app** (greenfield) — UI + orchestrator + herdr bridge. *New.*
...
1. Hook ingest + session storage: into kanban-api, or Tank-local SQLite then sync?
2. herdr socket API — documented/stable for programmatic drive, or do we wrap the CLI?
3. PR/merge: drive by *typing into claude* (max ToS purity) or direct gitea API calls (simpler)?
   Mixed?
4. Auto-naming model — which local model, and is it worth it vs. first-line-of-prompt?
5. Hermes migration scope — same milestone as Tank, or sequence after P3?
