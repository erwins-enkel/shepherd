...
| `standard`   | bwrap membrane | Agent confined to its worktree + git object store + read-only `~/.claude`; cannot read `~/.ssh`, sibling repos, or `$HOME` dotfile secrets; no privilege escalation. Opt-in for interactive sessions. |
...
**Network egress allowlist is not yet implemented** (tracked in [#551](https://github.com/erwins-enkel/shepherd/issues/551)) — `autonomous` is filesystem-confined only, not network-confined yet.
