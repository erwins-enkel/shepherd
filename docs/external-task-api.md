...
| `images`        | `string[]`                                                                                                                                             | —        | Staged **attachment** paths (any file type, not just images — the key name is kept for compatibility); ≤10 entries, each confined to the upload staging dir (see `POST /api/uploads`)                                                |
| `attachmentNames` | `string[]`                                                                                                                                           | —        | Optional display names for the `images` entries; when present its length must equal `images`. Each name is sanitized (path separators/control chars stripped, ≤160 chars) and falls back to `Attachment <n>`                        |
