...
| 0.8.2   | Protocol 20 is additive; the driven CLI/records and live behaviour are unchanged, while #1716 remains unfixed                                                                                                   | Admit protocol 20 by name, keep the external-registration path and the sandbox-status floor at 0.7.4                                                                                  | #2096                                            |
| 0.9.1   | Nothing on the driven surface moved (protocol stayed 22, schema only gained `pane.link.resolve`, no record or `--help` drift) — but the release's real changes hide behind UNCHANGED flags, which S4 cannot see | Read the notes, not just the diff: the 30s terminal-observer disconnect lands on the default-OFF socket terminal, whose `onAbnormalExit` reads a missing close record as version skew | herdr #3612, #4123; `docs/herdr-compat/0.9.1.md` |
