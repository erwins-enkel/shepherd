            ...
            {#each row.windows as win (win.key)}
              <div class="pcap-window">
                ...
                <span
                  ...
                  aria-valuetext={m.newtask_provider_capacity_meter_window_aria({
                ...
                <span class="pcap-value"
                  >{m.newtask_provider_capacity_free({ pct: win.remainingPct })}</span
                >
              </div>
            {/each}
          </div>
...
<style>
  ...
  .pcap-window {
    display: grid;
    grid-template-columns: minmax(3rem, 4rem) minmax(0, 1fr) auto;
    gap: 8px;
    align-items: center;
    min-width: 0;
