...
6. **Explanatory tooltips follow the rule in `CLAUDE.md`.** Use `$lib/tooltips/TooltipBody.svelte` and `TooltipExplanation` for a title, summary and labelled sections; `$lib/tooltips/statusTip.svelte` owns status-trigger behavior. Glossary explanations supply the same structure via their registry entry. Read the live explanatory-tooltip examples on `/design-system` before adding one. A longer plain string or merely wider panel does not meet the rule.
