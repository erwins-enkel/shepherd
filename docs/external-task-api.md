...
| `model`         | a Claude alias (`"fable" \| "claude-fable-5-1" \| "opus" \| "opus[1m]" \| "claude-opus-5" \| "claude-opus-5[1m]" \| "sonnet" \| "sonnet[1m]" \| "haiku"`), a Codex model id (with `agentProvider: "codex"`), or `null` | —        | Validated against the selected provider. Omit/`null`/`"default"` = that provider's default model; `[1m]` selects the 1M-context Claude variant. Bare aliases (`"opus"`, `"fable"`) track the latest model of that tier; the full model name (`"claude-opus-5"`, `"claude-fable-5-1"`) pins an exact version |
