...
public enum Loaded<Value: Equatable & Sendable>: Equatable, Sendable {
    case loading
    case ready(Value)
    case failed(String)
...
public enum DetailFeed: String, CaseIterable, Sendable {
...
@Observable
@MainActor
public final class DetailModel: AppExtension {
    public struct DiffPayload: Equatable, Sendable {
    ...
    public enum FilesSource: String, CaseIterable, Sendable { case scratchpad, worktree }

    public struct FilesPayload: Equatable, Sendable {
        public var source: FilesSource
        public var listing: BrowseListing
    }

    /// Every server read, as closures, so tests drive the model without a server.
    public struct Loaders: Sendable {
        public var activity: @MainActor (String) async throws -> [ActivityEntry]
        public var diff: @MainActor (String) async throws -> DiffResult
        public var annotations: @MainActor (String) async throws -> [DiffNote]
        public var scratchpad: @MainActor (String, String?) async throws -> BrowseListing
        ...
        public static func stubbed() -> Loaders {
            Loaders(
                activity: { _ in [] },
                diff: { _ in
                    DiffResult(
    ...
    public private(set) var activity: [String: Loaded<[ActivityEntry]>] = [:]
    public private(set) var diff: [String: Loaded<DiffPayload>] = [:]
    public private(set) var files: [String: Loaded<FilesPayload>] = [:]
    public private(set) var git: [String: Loaded<GitState?>] = [:]
    ...
    public init(loaders: Loaders) { self.loaders = loaders }
    ...
    public var isActive: Bool { alive }
    ...
    public func load(_ feed: DetailFeed, session id: String) async {
    ...
    public func browse(session id: String, source: FilesSource, path: String?) async {
    ...
    public func poll(_ feed: DetailFeed, session id: String) async {
    ...
    public func refreshGit(session id: String) async { await load(.git, session: id) }
    ...
    public func isRefreshing(_ feed: DetailFeed, session id: String) -> Bool {
