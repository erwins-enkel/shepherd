...
public struct SettingsSnapshot: Sendable {
    ...
    public var usage: UsageLimits?
    public var repos: [Components.Schemas.Repo]
}
...
@Observable @MainActor public final class SettingsModel: AppExtension {
    ...
    public var reviewer = ""
    public var merger = ""
    public var forkTarget = ""
    public private(set) var workspaceAction: String?
    public private(set) var workspaceTarget = ""
    private var pendingConfig: RepoConfigPatch?
    @ObservationIgnored private var pendingConfigCommit: (@MainActor (RepoConfig) -> Void)?
    public var roles: RepoRolesResult? {
        didSet {
            reviewer = roles?.roles.reviewer ?? ""
            merger = roles?.roles.merger ?? ""
        }
    }
    public var collaborators: RepoCollaborators?
    public var directories: DirectoryListing?
    public var verification: KeyVerification?
    public let tokens = SettingsTokensModel()
    @ObservationIgnored private var suppliedRecovery: BackendRecoveryModel?
    public var recovery: BackendRecoveryModel? { suppliedRecovery ?? app?.extension(BackendRecoveryModel.self) }
    public var diagnostics: DiagnosticsSnapshot? { recovery?.diagnostics ?? snapshot?.diagnostics }
    ...
    public func runDiagnostic(_ operation: @Sendable () async throws -> DiagnosticsSnapshot) async {
        guard !stopped, !busy else { return }
        busy = true; error = nil
        let activation = app?.activationGeneration
    ...
    public func replaceDiagnostics(_ value: DiagnosticsSnapshot) {
        if let recovery { recovery.replaceDiagnostics(value) }
        else { snapshot?.diagnostics = value }
    }
    public func patch(_ body: SettingsPatch, client: ShepherdClient) {
        run { try await client.patchSettings(body: body) }
    }
    public func selectRepo(_ path: String, client: ShepherdClient) {
        guard !stopped, !busy else { return }
    ...
    public func requestWorkspaceAction(_ action: String, config: RepoConfigPatch? = nil,
    ...
    public func cancelWorkspaceAction() {
        workspaceAction = nil; workspaceTarget = ""; pendingConfig = nil
        pendingConfigCommit = nil
    }
    public func applyWorkspaceAction(client: ShepherdClient) {
        guard !stopped, !busy else { return }
