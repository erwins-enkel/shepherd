...
| Design tokens       | `ui/src/app.css` — `--color-*` surfaces/text/accents, `--fs-*` scale, `--status-*`/`--wash-*`; recipes `.gbtn`/`.badge`/`.panel`/`.scrim`                                                                       | **Map** `--wf-*` → these (§3.3)                                     |
...
Each block is its own small component (`FileTreeBlock`, `DataModelBlock`, `ApiEndpointBlock`, `CalloutBlock`, …). `DiffBlock` factors out the existing `DiffFileBlock` per-file view; `CodeBlock`/`AnnotatedCodeBlock` wrap `highlightLines()`. All use `--color-*`/`--fs-*` tokens and the `.panel`/`.badge` recipes — no new design language.
