...
public final class ComposeModel {
    public let shaping: ShapeRoundModel
    public let attachments: AttachmentModel
    public let repoBranches: RepoBranchModel
    public var repoPath = "" {
        didSet {
    ...
    public var provider: AgentProvider = .claude { didSet {
    ...
    public var effort = "default" { didSet {
        if !normalizingRunConfig { effortTouched = true }
        normalizeRunConfig()
    } }
    public var runDefaults: ComposeRunConfig.Defaults { didSet { reconcileRunDefaults() } }
    ...
    public var research = false { didSet { if oldValue != research { shaping.discard() } } }
    ...
    public var planGateEnabled = false
    public var planGateTouched = false
    public var autopilotEnabled = false
    public var autopilotTouched = false
    public var sandboxProfile: Components.Schemas.SandboxProfile?
    private(set) var modeTouched = false
    private var designPreselected = false
    public var issueQuery = ""
    public var prompt = "" {
        didSet {
    ...
    public var filter: IssueFilterState {
        didSet {
    ...
    public var source: ComposeSource = .issues
    public var showFilters = false
    public var choosingFiles = false
    public var focusTarget = ""
    public var focusRevision = 0
    public func requestFocus(_ target: String) { focusTarget = target; focusRevision += 1 }
    public var expanded = false
    public private(set) var listing: IssueListing?
    private(set) var issues: [Issue] = []
    private var commandListings: [AgentProvider: [SlashCommand]] = [:]
    public var commands: [SlashCommand] { commands(for: provider) }
    public private(set) var viewer: String?
    public private(set) var epicParents: Set<Int> = []
    ...
    public struct ProviderConstraint: Equatable {
        public let token: String
        public let provider: AgentProvider
    }
    public private(set) var providerConstraint: ProviderConstraint? { didSet { normalizeRunConfig() } }
    ...
    public func selectProviderManually(_ provider: AgentProvider) {
    ...
    public var modeLocked: Bool { research || epicAuthoring || plain }
    public var sandboxLocked: Bool { research || epicAuthoring }
    public var shapingOffered: Bool { mode == .code }

    public var shapeBlocker: String? {
        ShapeRoundModel.blocker(running: shaping.running || shaping.composing, mode: mode,
                                repoPath: repoPath, prompt: prompt)
    }

    public func startShaping() async {
        normalizeRunConfig()
    ...
    public func useBrief(_ answers: [RawAnswer]) async {
        if let brief = await shaping.useBrief(answers) { prompt = brief }
    }

    public var guardExplanation: String? {
        switch mode {
        case .code: nil
    ...
    public func setMode(_ next: ComposeMode) {
        modeTouched = true
    ...
    public func commands(for provider: AgentProvider) -> [SlashCommand] { commandListings[provider] ?? [] }

    public func commandProvider(at caret: String.Index) -> AgentProvider {
    ...
    public func loadCommands(provider requestedProvider: AgentProvider? = nil) async {
    ...
    public func openRepoPicker() { repoBranches.presentedPicker = .repo }
    public func openBranchPicker() { repoBranches.presentedPicker = .branch }
    public func cycleRepo(_ direction: Int, repos: [Repo]) {
    ...
    public var openCount: Int? { listing?.slug != nil && !issuesFailed ? issues.count : nil }
    public var activeIssue: Issue? { attachedRepoPath == repoPath ? attached : nil }
    public var filteredIssues: (visible: [Issue], emptiedBy: IssueFilter.Stage?) {
        IssueFilter.apply(issues, viewer: viewer, epicParents: epicParents, subIssues: subIssues, state: filter)
    }
    public var authors: [String] { Array(Set(issues.compactMap(\.author))).sorted { $0.localizedCaseInsensitiveCompare($1) == .orderedAscending } }
    ...
    public func pickIssue(_ issue: Issue) {
    ...
    public func removeIssue() { attached = nil; attachedRepoPath = nil }
    public func pickIssueFromSearch(_ issue: Issue, caret: String.Index) {
        if let trigger = Self.trigger(in: prompt, caret: caret), trigger.symbol == "#" {
            prompt.removeSubrange(trigger.range)
        }
        pickIssue(issue)
    }
    public func issueMatches(_ query: String) -> [Issue] {
    ...
    public static func commandMatches(_ commands: [SlashCommand], query: String) -> [SlashCommand] {
    ...
    public static func isInsertable(_ command: SlashCommand) -> Bool {
    ...
    @discardableResult
    public func pickCommand(_ command: SlashCommand, caret: String.Index? = nil) -> String.Index {
    ...
    public func allowsProvider(_ provider: AgentProvider) -> Bool {
