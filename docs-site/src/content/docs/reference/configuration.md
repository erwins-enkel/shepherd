...
| `SHEPHERD_HERDR_SOCKET` | `0` (off) | Opt-in: read agent state over herdr's native Unix-socket JSON-RPC API instead of shelling out to the `herdr` CLI for every call (issue #1529). Writes still go through the CLI. Default-off because the socket protocol is still preview-unstable; the driver falls back to the CLI on any protocol mismatch, so enabling it is reversible |
