...
| `SHEPHERD_DISABLED_PLUGINS`     | _(none)_                        | Comma-separated `name@marketplace` plugin ids to disable for spawned agents, process-scoped via `--settings enabledPlugins` (never touches your global config or interactive sessions). Trims those plugins' always-listed skill descriptions from the agent's per-turn context (re-read every turn). Purely subtractive — omitted plugins stay enabled, so superpowers/context7 are unaffected. e.g. `vercel@claude-plugins-official,resend@claude-plugins-official`                                                                                                                                                                                  |
