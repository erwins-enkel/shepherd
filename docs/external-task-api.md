...
| `effort`        | `"low" \| "medium" \| "high" \| "xhigh" \| "max"`, `"default"`, or `null`                                                                                                                                              | —        | Reasoning-effort tier. Omit/`null`/`"default"` = the provider CLI's own default (no effort flag). Passed through as `--effort` for Claude; for Codex `xhigh` passes through while `max` clamps down to `high` at spawn for provider-wide compatibility                                                      |
