...
struct NewSessionProviderSeedTests {
    @Test func aLateDefaultIsAppliedWhileTheProviderIsUnedited() {
        #expect(ProviderSelection.arrivingProviderDefault(.codex, alreadySeeded: false) == .codex)
    }

    @Test func aLateDefaultIsIgnoredOnceTheProviderWasSeededOrEdited() {
        #expect(ProviderSelection.arrivingProviderDefault(.codex, alreadySeeded: true) == nil)
    }

    @Test func settingsArrivingWithoutAProviderChangeNothing() {
        #expect(ProviderSelection.arrivingProviderDefault(nil, alreadySeeded: false) == nil)
    }
}
}
