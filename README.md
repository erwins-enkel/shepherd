...
| `standard`   | bwrap membrane | Agent confined to its worktree + git object store + read-only `~/.claude`; blocks `~/.ssh`, `~/.aws`, sibling repos, and other `$HOME` dotfiles, and clears inherited env secrets; no privilege escalation. (The `gh` and `claude` auth tokens it needs stay readable — see the egress caveat below.) Opt-in for interactive sessions. |
| `autonomous` | bwrap membrane | Same filesystem/process membrane as `standard`. **Required for `auto=true` (drain/autopilot) sessions** — Shepherd refuses to auto-spawn an agent with a weaker profile.                                                                                                                                                               |

**Network egress allowlist is not yet implemented** (tracked in [#551](https://github.com/erwins-enkel/shepherd/issues/551)) — `autonomous` is filesystem-confined only, not network-confined yet. Because the membrane deliberately keeps the `~/.config/gh` token and `~/.claude` OAuth credentials readable (the agent needs them to push and to talk to Anthropic), and outbound egress is unrestricted, **those auth tokens remain exfiltratable** by a hijacked agent until egress lands. The membrane shrinks the blast radius (no `~/.ssh`/`~/.aws`/sibling-repo/other-`$HOME`-dotfile access, no inherited env secrets), it does not eliminate it for the tokens the session itself requires.
