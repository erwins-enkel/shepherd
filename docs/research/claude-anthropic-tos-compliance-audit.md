...
| **R3** | OAuth token exfiltration                                    | `~/.claude/.credentials.json` is deliberately readable inside the agent sandbox; egress allowlist (#551) not yet implemented. Untrusted content + readable token + `--dangerously-skip-permissions` = an exfiltration path; a leaked token used elsewhere is exactly the _prohibited_ "consumer OAuth outside Claude Code" case. | 🟠 Medium   | Partially mitigated; **close #551**                        |
...
- **Unattended automation surfaces:** auto-drain (spawns agents from a backlog), autopilot (steers a blocked agent by injecting keystrokes), critic-on-PR, plan-gate, merge train, distiller, recap. Concurrency is bounded per-repo by `maxAuto` and gated by a `usageCeilingPct` that pauses drain near the subscription's usage limit (`src/drain-core.ts`, `src/config.ts`).
- **Data egress to Anthropic:** task prompts, GitHub/Gitea issue + PR text, PR diffs, git history, repo code, uploaded images, operator-curated house rules. The autonomous-mode egress allowlist (`src/egress.ts`) permits `api.anthropic.com` + `statsig.anthropic.com` (and GitHub hosts) — **but the allowlist is not yet enforced** (README + #551); today autonomous mode is filesystem-confined only, and the OAuth token stays readable so agents can authenticate.
...
- **R3 — Token exfiltration.** Until #551 lands, untrusted content (web/repo) + a readable OAuth token + `--dangerously-skip-permissions` is an exfiltration path. A leaked token used elsewhere is the _prohibited_ "consumer OAuth outside Claude Code" case. The egress allowlist already scopes outbound to `api.anthropic.com` — completing it confines the token to Anthropic and largely closes this. **Prioritise #551.**
...
3. **Close #551 (egress allowlist).** Highest-leverage security+ToS fix: confines the OAuth token to `api.anthropic.com`, neutralising R3 and most of R4's exfiltration surface.
